...
def load(file_object, *transformers, **kwargs):
    ...
    marshaller = JavaObjectUnmarshaller(
        file_object, kwargs.get('bytes_callback'))
...
class JavaObjectUnmarshaller(JavaObjectConstants):
    ...
    """
    def __init__(self, stream, bytes_callback=None):
        """
        Sets up members
        ...
        """
        self.bytes_callback = bytes_callback
    ...
    def do_array(self, parent=None, ident=0):
        ...
        if type_char == self.TYPE_OBJECT or type_char == self.TYPE_ARRAY:
            ...
            if self.bytes_callback is not None:
                array = self.bytes_callback(self.object_stream, size)
            else:
                array = self.object_stream.read(size)
        else:
            for _ in range(size):
